...
export const RecordingGuildStep: React.FC<RecordingGuildStepProps> = React.memo((props) => {
    ...
    const renderLabel = React.useMemo(() => {
        switch (stepName) {
            case 'intro':
                return `Định vị khuôn mặt của bạn trong khung hình camera. Sau đó làm theo từng bước theo hướng dẫn (gật đầu, xoay đầu, chớp mắt và đóng mở miệng), mỗi bước sẽ có 5 giây thực hiện`;
            case 'pitch_head':
